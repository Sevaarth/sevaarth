...
import { Heart, Users, Share2, ChevronDown, ChevronUp, Calendar, MapPin, ArrowRight } from 'lucide-react';
...
const InitiativeCard = ({ 
  ...
  const [supportCount, setSupportCount] = useState(() => Math.floor(Math.random() * 100) + 50);
    ...
    <Card className="w-full transition-all duration-300 hover:shadow-xl overflow-hidden bg-white">
      ...
      <CardContent>
        <div id={`description-${title.replace(/\s+/g, '-').toLowerCase()}`}>
          <p className={`text-gray-600 ${isExpanded ? '' : 'line-clamp-3'} transition-all duration-300`}>
            {description}
          </p>
        </div>
        <button
      ...
      <CardFooter className="border-t pt-4 flex flex-wrap justify-between items-center gap-2">
        ...
        >
          Get Involved <ArrowRight size={16} className="ml-1 inline" aria-hidden="true" />
        </Button>
      </CardFooter>
    </Card>
  );
  ...
  const cards = [
    {
      ...
      description: "The primary focus of Sevaarth is to assist seniors who are experiencing financial difficulties. Many elderly individuals live on fixed incomes, which can make it challenging to afford basic necessities. By providing support, we aim to alleviate some of the financial burdens they face.",
    },
    {
      ...
      description: "Education is a powerful tool that can help lift families out of poverty. By providing educational support to children, we aim to create opportunities for them to succeed and build a better future. Every child has unique talents and abilities.",
    },
    {
      ...
      description: "Collecting and distributing clothes to those in need, fostering a spirit of giving and community support. We ensure dignity and respect in our distribution process while meeting the essential clothing needs of vulnerable populations.",
    },
    {
      ...
      description: "Committed to enhancing healthcare services and access for communities through innovative and sustainable health programs. We focus on preventive care, health education, and improving access to quality medical services.",
    },
    {
      ...
      description: "Empowering women through education, skill development, and leadership opportunities. Our programs focus on financial literacy, entrepreneurship training, and creating safe spaces for women to grow and thrive in their communities.",
    },
    {
      ...
      description: "Promoting environmental sustainability through tree planting and cleanliness drives. We organize regular community events to plant trees, maintain public spaces, and raise awareness about environmental conservation.",
    },
      ...
      <div className="max-w-6xl mx-auto px-4">
        <div className="text-center mb-12">
          ...
          <p className=" text-lg max-w-2xl mx-auto">
            Join us in making a difference. Every action counts, every life matters.
          </p>
        </div>
